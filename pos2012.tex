...
As we explain in detail below, this optimization forces us to probe a different renormalization scheme for each $\be_F$, so that the bare step-scaling function we obtain is a composite of many different discrete \be functions.
...
In contrast, our 8-flavor results for $s_b$ are all significantly different from zero. % for $\be_F$ ranging from relatively weak couplings to the \Sb lattice phase.
...
Because the Wilson flow moves the system in the infinite-dimensional space of lattice-action terms without changing the lattice scale, we can use it to approach the renormalized trajectory corresponding to a fixed RG blocking transformation. % Try to clarify ``action-space''
By optimizing the flow time $t_f$ for fixed RG blocking transformation at each coupling, we can now carry out the two-lattice matching without a need for further optimization.
...
We determine whether the blocked systems have flowed to the same point on the renormalized trajectory by matching several short-range gauge observables: the plaquette, all three six-link loops and the $2\X2$ eight-link loop.
...
At weaker couplings (larger $\be_F$), the step-scaling functions approach the perturbative predictions $s_b \approx 0.6$ (0.3) for $N_f = 8$ (12).
...
We also use the same $24^3\X48$ and $12^3\X24$ lattices with fermion masses $m = 0.0025$ and $m = 0.01$, respectively.
